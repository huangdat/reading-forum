...
public class novelPageController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet novelPageController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet novelPageController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
        ...
        if (path.equals("/novelPageController")) {
            ...
            if (path.equals("/novelPageController/novelpage")) {
                ...
                if (path.startsWith("/novelPageController/novel_id/")) {
                    ...
                    // check khi bấm like 
                } else if (path.startsWith("/novelPageController/favorite/")) {
                    String[] s = path.split("/");
                    String id = s[s.length - 1];
...
//                            daoPage.updateFavouriteNovel(Integer.parseInt(id), 0);
                ...
                } else if (path.startsWith("/novelPageController/comment/")) {
                    String[] s = path.split("/");
                    String id = s[s.length - 1];
                    ...
                    try {
                        while (rs.next()) {
                            ...
                            response.sendRedirect("/novelPageController/novel_id/"+id);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else if (path.startsWith("/novelPageController/rating/")) {
                    String[] s = path.split("/");
                    String id = s[s.length - 1];
                ...
                } else if (path.startsWith("/novelPageController/chapter/")) {
                    String[] s = path.split("/");
